...
- **RLS is the enforcement locus for every write, not just the review-stage transitions.**
  Master-data, settings, and branding writes are admin-only; quote content edits are
  owner-or-admin; `Pending Approval → Approved` and `Pending Approval → Draft` are
  admin-only; reads are flat (any authenticated user).
